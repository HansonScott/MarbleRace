...
public class ColorPicker : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    private void Awake()
    {
        ...
        // since this is a one-time thing to awake, let's set it to a random color
        // NOTE: this calls ApplyColor in the property, so no need to call it again.
        this.color = CreateRandomVibrantColor();

        //ApplyColor();
    }

    private Color CreateRandomVibrantColor()
    {
        return Color.HSVToRGB(UnityEngine.Random.value, 1, 1);
    }
    ...
    private bool WrongShader()
    {
        ...
        if(image.material == null) { return true; }
        if(image.material.shader == null) { return true; }
        if(image.material.shader.name != colorPickerShaderName) { return true; }
        return false;
